...
static async generateImage(prompt, config = null) {
    ...
    let url = cfg.imageApiUrl || 'https://image.novelai.net/ai/generate-image';
    ...
    // 获取模型
    let model = cfg.imageModel || 'nai-diffusion-4-curated-preview';

    // 处理尺寸
    const sizeStr = cfg.imageSize || '1024x1024';
    const [width, height] = sizeStr.split('x').map(Number);

    // 处理提示词模板
    let finalPrompt = prompt;
    if (cfg.imagePromptTemplate) {
        if (cfg.imagePromptTemplate.includes('{prompt}')) {
            finalPrompt = cfg.imagePromptTemplate.replace('{prompt}', prompt);
        } else {
            finalPrompt = cfg.imagePromptTemplate + ', ' + prompt;
        }
    }

    // 反面提示词
    const negativePrompt = cfg.imageNegativePrompt || 'lowres, {bad}, error, fewer, extra, missing, worst quality, jpeg artifacts, bad quality, watermark, unfinished, displeasing, chromatic aberration, signature, extra digits, artistic error, username, scan, [abstract]';

    // 高级参数
    const sampler = cfg.imageSampler || 'k_euler_ancestral';
    const steps = cfg.imageSteps || 28;
    const cfgScale = cfg.imageCfg || 5;
    const noiseSchedule = cfg.imageNoiseSchedule || 'native';
    const seed = cfg.imageSeed || Math.floor(Math.random() * 9999999999);

    // SMEA 处理
    let smea = false;
    let smeaDyn = false;
    if (cfg.imageSmea === 'SMEA') {
        smea = true;
    } else if (cfg.imageSmea === 'SMEA+DYN') {
        smea = true;
        smeaDyn = true;
    }
    ...
    try {
        ...
        if (contentType && contentType.includes('application/zip')) {
            ...
            // 使用 JSZip 解压（需要引入库）或直接尝试解析
            // 简化处理：NovelAI 的 zip 通常只有一个 png 文件
            try {
                ...
                if (JSZip) {
                    ...
                    if (imageFile) {
                        const imgData = await zip.files[imageFile].async('base64');
                        return 'data:image/png;base64,' + imgData;
                    }
                }
            } catch (zipErr) {
                console.warn('ZIP解析失败尝试其他方式', zipErr);
            }
            ...
            if (pngStart !== -1) {
                ...
                return await window.Utils.fileToBase64(pngBlob);
            }

            throw new Error('无法解析 ZIP 响应中的图片');
        }
        ...
        throw new Error('未知的 Image API 响应格式');
    } catch (e) {
