...
(createBtn, settingsBtn);
    ...
    async renderHome() {
        ...
        list.innerHTML = '';
        // ========== 修复：确保生成按钮悬浮在右下角 ==========
const existingFloatBtn = document.getElementById('genHomeFloatBtn');
if (!existingFloatBtn) {
    const floatBtn = document.createElement('div');
    floatBtn.id = 'genHomeFloatBtn';
    floatBtn.style.cssText = 'position:fixed;bottom:80px;right:20px;width:50px;height:50px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:50%;display:flex;justify-content:center;align-items:center;cursor:pointer;z-index:1000;box-shadow:0 4px 15px rgba(102,126,234,0.4);';
    floatBtn.innerHTML = '<i class="fas fa-magic" style="color:#fff;font-size:18px;"></i>';
    floatBtn.title = '生成帖子';
    floatBtn.onclick = () => this.generatePosts(null);
    document.getElementById('forumApp').appendChild(floatBtn);
}
// ========== 悬浮按钮修复结束 ==========
