...
class ForumApp {
    ...
    async generateBoards() {
        ...
        const settings = this.store.get().settings;
let prompt = `【论坛帖子生成 - 角色独立性测试】

世界观：${settings.worldSetting}
板块：${boardName}
${memoryPrompt}
${query ? `关键词：${query}` : ''}

【可用角色】
QQ好友：${friends.map(f => `${f.name}(${f.persona?.substring(0,40) || '未知'})`).join('、')}
或路人（随机网友）

【已存在的作者】
${existingAuthors.join('、') || '无'}

【生成要求】
生成5-8个论坛帖子。

【核心规则】
1. 每个帖子的作者性格、说话风格必须完全不同
2. 同一个作者不能连续发两个帖子
3. 内容要有实质：不要空洞的标题党要有真实感
4. 风格多样化：有人认真科普、有人发牢骚、有人分享日常、有人求助、有人抬杠
5. 标题和内容的语气要匹配该作者性格
...
[
  {
    "title": "标题",
    "content": "正文内容（200字以上要有细节）",
    "author": "作者名",
    "likes": 随机点赞数,
    "poll": null或{"question":"投票问题","options":["选项1","选项2"]},
    "comments": [
      {"author":"评论者名","content":"评论内容"}
    ]
  }
]

【风格示例】（每个帖子必须完全不同）
帖子A：严肃科普型 - "关于XXX的几个误区"
帖子B：吐槽抱怨型 - "草了今天真是服了"
帖子C：分享日常型 - "刚刚在街上看到..."
帖子D：求助提问型 - "有人知道XXX吗急"
帖子E：争议引战型 - "我就说XXX不行吧"
