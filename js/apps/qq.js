...
if(!isGroup) {
    ...
    if(freq === 'auto') {
        ...
        shouldGenerateMoment = Math.random() < 0.3;
    }
}

// 检查是否应该生成角色聊天记录
const shouldGenerateOtherChats = !isGroup && enableOtherChats && otherFriendsForChat.length > 0 && Math.random() < 0.3;
            ...
            if(!isNaN(amt) && amt > 0) {
                ...
                if(m.senderId === 'user') {
                    ...
                    d.wallet.history.unshift({
                        date: new Date().toLocaleString(),
                        amount: `-${amt.toFixed(2)}`,
                        reason: `${target.name}领取了你的${m.subType === 'redpacket' ? '红包' : '转账'}`
                    });
                }
            }
        }
