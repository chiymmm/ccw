...
// ========== 在 renderMessages() 方法中替换消息内容渲染部分 ==========
    ...
    // 🔴 新增：@ 高亮处理
    if(this.currentChatType === 'group') {
        displayContent = displayContent.replace(/@(\S+)/g, (match, name) => {
            return `<span style="color:#576b95;font-weight:600;">@${name}</span>`;
        });
    }
...
async continueGroupChat() {
    ...
    try {
        await this.startAIGroupChat(this.currentChatId);
...
async generateCustomPlot() {
    window.Utils.showCustomDialog({
        ...
        if(res.action && res.inputs.plotDesc) {
            ...
            try {
                await this.startAIGroupChatWithPlot(this.currentChatId, res.inputs.plotDesc);
...
只返回JSON数组。`;
    ...
    try {
        ...
        console.error('Group chat with plot failed', e);
        throw e;
    }
}
