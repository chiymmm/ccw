...
// ========== 新增：角色退回/拒绝红包转账 ==========
const rejectMatch = content.match(/\[ACTION:REJECT:(\d+)\]/);
if(rejectMatch) {
    ...
    if(targetMsg && !targetMsg.claimed && !targetMsg.rejected) {
        ...
        this.store.update(d => {
            ...
            if(m) {
                ...
                if(!isNaN(amt) && amt > 0 && m.senderId === 'user') {
                    // 退回给用户
                    d.wallet.balance = (parseFloat(d.wallet.balance) + amt).toFixed(2);
                    d.wallet.history.unshift({
                        date: new Date().toLocaleString(),
                        amount: `+${amt.toFixed(2)}`,
                        reason: `${target.name}退回${m.subType === 'redpacket' ? '红包' : '转账'}`
                    });
                }
            }
        });
        ...
        // 🔴 修复：使用预先保存的变量
        this.sendSystemMessage('system', `${target.name} 退回了你的${msgSubType === 'redpacket' ? '红包' : '转账'}`, null, false);
    }

    finalContent = finalContent.replace(rejectMatch[0], '');
}
...
if(claimMatch) {
    ...
    if(targetMsg && !targetMsg.claimed) {
        const msgSubType = targetMsg.subType; // 保存类型
        const msgData = targetMsg.data; // 保存金额
            ...
            if(m) {
                ...
                if(!isNaN(amt) && amt > 0) {
                    ...
                    const historyItem = d.wallet.history.find(h =>
                        ...
                        h.amount === `-${amt.toFixed(2)}`
                    );
                    if(historyItem) {
                        historyItem.reason = historyItem.reason.replace('（待领取）', `（${target.name}已领取）`);
                    }
                }
            }
        });
        ...
        // 🔴 修复：使用预先保存的变量
        this.sendSystemMessage('system', `${target.name} 领取了你的${msgSubType === 'redpacket' ? '红包' : '转账'}`, null, false);
    }
...
acceptTransfer(msgId) {
    const self = this;
    const msg = this.store.get().messages[this.currentChatId].find(m => m.id == msgId);
